...
report_file_name="QID_Service_Report.html"
log_file_name="QID_Service_Log.log"
sys.path.insert(0, project_dir)
from src.ConfigParser import ConfigParser
from src.SendEmail import SendReportEmail
...
host = cfg.safe_get('AUTOMATION.RUN', 'URL').split(".", 1)[1]
...
def run_test():
    ...
    arglist.append("-rap")
    #arglist.append("--tests-per-worker=auto")
    #arglist.append("-n=2")
    #arglist.append("--dist=loadfile")
#    arglist.append("--dist=loadscope")
    if parsed.modules is not None:
        module = " or ".join(parsed.modules.split(","))
        print("executing test cases from module: " + module)
...
def send_report():
    ...
    if to is not None:
        subject=host+" - QBEP-"+qbep_version+" ("+ parsed.modules +")"
